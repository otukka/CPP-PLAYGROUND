...
void tdigest<Values, Weight>::merge()
{
    ...
    if (run_forward)
    {
        ...
        min_val
            = std::min(min_val, inputs.front().weight == 1 ? inputs.front().mean : std::numeric_limits<Values>::max());

        max_val
            = std::max(max_val, inputs.back().weight == 1 ? inputs.back().mean : std::numeric_limits<Values>::min());
    }
    else
    {
        ...
        // Update min/max values only if sorted first/last centroids are single points.
        min_val
            = std::min(min_val, inputs.back().weight == 1 ? inputs.back().mean : std::numeric_limits<Values>::max());

        max_val
            = std::max(max_val, inputs.front().weight == 1 ? inputs.front().mean : std::numeric_limits<Values>::min());
    }
    ...
    auto compress_fn = [&inactive, new_total_weight, &k1, normalizer, &next_q_limit_weight, &weight_so_far,
                        &weight_to_add, &mean_to_add](const centroid_t& current) {
...
double tdigest<Values, Weight>::cumulative_distribution(Values x) const
{
    ...
    auto cdf_fn = [x, &weight_so_far, &cdf, total_weight = active->total_weight](const centroid_t& left,
        ...
        if (left.mean <= x && x < right.mean)
        {
            ...
            cdf = lerp(base, base + delta_weight, static_cast<double>((x - left.mean)) / (right.mean - left.mean))
                  / total_weight;
            return (true);
        }
